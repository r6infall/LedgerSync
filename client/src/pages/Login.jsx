...
export default function Login() {
  ...
  const [googleForm, setGoogleForm] = useState({ role: 'buyer', businessName: '', gstin: '' });
  ...
  useEffect(() => {
    if (!googleForm.gstin) {
      setGstinStatus('');
    } else if (gstinRegex.test(googleForm.gstin.toUpperCase())) {
  ...
  const setGoogle = (key) => (e) => setGoogleForm(f => ({ ...f, [key]: e.target.value }));
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      if (data.isNewUser) {
        ...
        navigate(`/${data.user?.role || 'buyer'}/dashboard`);
      }
    } catch (err) {
  ...
  const finalizeGoogleRole = async (e) => {
    e.preventDefault();
    ...
    try {
      await updateUser({ 
        ...
        businessName: googleForm.businessName, 
        gstin: googleForm.gstin ? googleForm.gstin.toUpperCase() : '' 
      });
      navigate(`/${googleForm.role}/dashboard`);
    } catch (err) {
        ...
        {googleNewUser ? (
          <div>
            <div style={{ textAlign: 'center', marginBottom: '24px' }}>
              <h3 style={{ fontSize: '15px', fontWeight: 600, color: '#1A1A1A', marginBottom: '8px' }}>One Last Step</h3>
              <p style={{ fontSize: '12px', color: '#555' }}>Please complete your business profile.</p>
            </div>
            <form onSubmit={finalizeGoogleRole}>
              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', fontSize: '11px', color: '#555', fontWeight: 500, marginBottom: '6px' }}>I am a</label>
                <select
                  value={googleForm.role} onChange={setGoogle('role')} required
                  style={{ width: '100%', border: '1px solid #E8E5E0', borderRadius: '5px', padding: '9px 12px', fontSize: '13px', background: '#FAFAF8', outline: 'none', cursor: 'pointer', appearance: 'auto' }}
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <input
                  type="text" placeholder="Sharma Enterprises" value={googleForm.businessName} onChange={setGoogle('businessName')}
                  style={{ width: '100%', border: '1px solid #E8E5E0', borderRadius: '5px', padding: '9px 12px', fontSize: '13px', background: '#FAFAF8', outline: 'none' }}
                  onFocus={e => e.target.style.borderColor = '#B8935A'} onBlur={e => e.target.style.borderColor = '#E8E5E0'}
                />
              </div>
              <div style={{ marginBottom: '24px' }}>
                ...
                <input
                  type="text" placeholder="27ABCDE1234F1Z5" value={googleForm.gstin} onChange={setGoogle('gstin')}
                  style={{ width: '100%', border: '1px solid #E8E5E0', borderRadius: '5px', padding: '9px 12px', fontSize: '13px', background: '#FAFAF8', outline: 'none', fontFamily: "'JetBrains Mono', monospace", textTransform: 'uppercase' }}
                  onFocus={e => e.target.style.borderColor = '#B8935A'} onBlur={e => e.target.style.borderColor = '#E8E5E0'}
                />
                {googleForm.gstin ? (
                  gstinStatus === 'valid' ? (
                  ...
                  <div style={{ fontSize: '10px', color: '#BBBBBB', marginTop: '4px' }}>Format: 27ABCDE1234F1Z5</div>
                )}
              </div>
